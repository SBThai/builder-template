# Pipeline

> Lightweight CRM. Anonymized descriptors only. Keep real names/emails in a private
> place outside this repo. Update stage + next action every time you touch a lead.

**Stages:** Lead → Contacted → Conversation → Trial/Proposal → Won / Lost

| Lead (anonymized) | Source | Stage | Next action | By when | Notes |
|---|---|---|---|---|---|
| e.g., HR manager, 50-person SaaS co | LinkedIn DM | Conversation | Send trial link | 2026-09-20 | Wants SSO eventually |

## Closed

| Lead | Won/Lost | Why (their words if possible) | Date |
|---|---|---|---|
